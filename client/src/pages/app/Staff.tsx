...
export default function StaffPage() {
  ...
  const load = () => { setLoading(true); api.get<any[]>('/staff').then(setItems).catch(console.error).finally(() => setLoading(false)); };

  useEffect(() => { api.get<any[]>('/schools').then(setSchools).catch(console.error); load(); }, []);

  const openCreate = () => { setEditing(null); setForm({ name: '', schoolId: schools[0]?.id, status: 'active' }); setModalOpen(true); setError(''); };
  const openEdit = (item: any) => { setEditing(item); setForm({ ...item }); setModalOpen(true); setError(''); };
  ...
  const handleDelete = async (id: string) => { if (!confirm('Delete this staff member?')) return; await api.delete(`/staff/${id}`); load(); };

  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-3xl font-bold text-gray-900">Staff</h1>
          <p className="text-gray-500 mt-1">Manage support staff across schools</p>
        </div>
        <button onClick={openCreate} className="btn-primary"><Plus className="w-5 h-5 mr-2" /> Add Staff</button>
      </div>
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center">
          ...
          <h3 className="font-display text-xl font-bold text-gray-900 mb-2">No staff members yet</h3>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item) => (
            <div key={item.id} className="card-hover p-6">
              ...
              <p className="text-sm text-purple-600 font-medium">{item.designation}</p>
              {item.duties && <p className="text-sm text-gray-500 mt-2">{item.duties}</p>}
              {item.phone && <p className="text-sm text-gray-500 mt-2">📞 {item.phone}</p>}
            </div>
          ))}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setModalOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-xl w-full" onClick={(e) => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <h2 className="font-display text-xl font-bold">{editing ? 'Edit Staff' : 'Add Staff'}</h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Duties</label>
                <textarea value={form.duties || ''} onChange={(e) => setForm({ ...form, duties: e.target.value })} className="input-field" rows={3} />
              </div>
            </div>
