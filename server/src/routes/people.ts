...
import { requireAuth, requireWrite, logAction } from '../auth';
...
r.get('/students', (req: Request, res) => {
  ...
  const isAdmin = ['super_admin', 'admin', 'principal'].includes((req as any).user?.role);
...
r.get('/students/:id', (req: Request, res) => {
...
r.post('/students', requireWrite, (req: Request, res) => {
  const b = req.body;
...
r.put('/students/:id', requireWrite, (req: Request, res) => {
  const b = req.body;
  if (b.aadhaar && String(b.aadhaar).replace(/\D/g, '').length !== 12)
    return res.status(400).json({ error: 'Aadhaar must be exactly 12 digits' });
...
r.delete('/students/:id', requireWrite, (req: Request, res) => {
...
r.post('/teachers', requireWrite, (req: Request, res) => {
  const b = req.body;
...
r.put('/teachers/:id', requireWrite, (req: Request, res) => {
  const b = req.body;
...
r.delete('/teachers/:id', requireWrite, (req: Request, res) => {
...
r.post('/staff', requireWrite, (req: Request, res) => {
  const b = req.body;
...
r.put('/staff/:id', requireWrite, (req: Request, res) => {
  const b = req.body;
...
r.delete('/staff/:id', requireWrite, (req: Request, res) => {
...
r.post('/smc', requireWrite, (req: Request, res) => {
  const b = req.body;
  if (b.id) {
...
r.delete('/smc/:id', requireWrite, (req: Request, res) => {
...
r.post('/attendance', requireWrite, (req: Request, res) => {
...
r.post('/teacher-attendance', requireWrite, (req: Request, res) => {
  const { school_id, date, records } = req.body;
